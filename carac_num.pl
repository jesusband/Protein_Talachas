...
die "usage: <carac_num.pl> <ranga.file> <output.file> <firstposition>" unless @ARGV == 3;

my $entrada=$ARGV[0];
my $salida=$ARGV[1];
my $c=$ARGV[2]; #contador inicial
...
my @ID_sec=split(/\t/,$_); #se separa la linea entre nombre y secuencia
	...
	if($elem =~ /\w/){printf NRN "$elem%03d|", $c; $c++;} #aqui el caracter "se convierte" en número (sólo si es una letra) y se imprime en el nuevo archivo
