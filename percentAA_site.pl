#!/usr/bin/perl
# freqsite_calculator.pl by Jesús Banda
# This script calculates the frequency of each residue per site of a ran multiple sequence alignment.
use strict; use warnings;

die "usage: percentAA_calc.pl <msa_file.ran>\n" unless @ARGV == 1;
...
for(my $site = 1; $site <= $tam; $site++){
    ...
    chomp $nogap;
    if($nogap =~ m/[A-Z]/){ #if the caracter is an aminoacid, then and just then, it will be considered
        my %resi = (); #v2
